...
- 工数入力画面の編集操作は Undo / Redo（ショートカットまたはUIボタン）に対応し、直前の変更を取り消したりやり直したりできる。
...
- ログイン後にユーザー設定をストアへ読み込み、存在しない場合は初期化APIを呼び出してから値を保持する（各画面ではストア参照のみで済む）。

### 3.10. ユーザー削除機能

- プロフィール画面にアカウント削除フローを設け、削除時は確認ダイアログでユーザーに最終確認を求める。
- 削除リクエストは Supabase Auth のユーザー削除 API へ委譲し、連鎖削除は DB の `CASCADE` 設定に任せる。
- ユーザー削除完了後は Supabase Storage（`avatars` バケット）の当該ユーザー用フォルダごと削除し、プロフィール画像など関連ファイルをすべて除去する。

### 3.11. 未認証時の画面遷移とログアウト

- 未認証状態ではログイン画面を表示せず、LP（ランディングページ）を表示する。
- LP には新規登録画面とログイン画面へ遷移できる導線（ボタンまたはリンク）を設ける。
- 認証済みの状態からログアウトした場合は、ログイン画面へ遷移する。
