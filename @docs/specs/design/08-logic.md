...
2. 画面読み込み時には `GET /api/projects`、`GET /api/tasks`、`GET /api/effort/draft` を呼び出し、案件・タスク候補を初期化するとともに、未送信の内容があればフォームへ復元する（ドラフトが存在しない場合は空状態から開始する）。ドラフトレスポンスには `client_updated_at` が含まれるため、クライアントは自分が保持する時刻と比較して最新か判定できる。
