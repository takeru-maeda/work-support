...
- **工数一覧画面 (`/efforts`)**
  ...
  - 画面遷移時に `localStorage` に保存されている絞り込み・並び替え条件を読込み、条件が存在する場合は `GET /api/reports/work-records` をその条件で実行して初期表示を行う（未保存の場合はデフォルト条件で取得する）。
  - 絞り込みボタン押下時は現在のフォーム入力値を `GET /api/reports/work-records` のクエリに反映して再取得し、使用した条件を `localStorage` に保存する。
