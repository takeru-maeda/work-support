...
- **ホーム画面 (`/`)**
  ...
  - 未認証状態でアクセスされた場合は LP（ランディングページ）を表示する。
- **工数登録画面 (`/efforts/new`)**
...
- **ランディングページ (`/` または `/landing`)**
  - 未認証時のデフォルト表示。サービス概要と「ログイン」「新規登録」への CTA ボタンを配置する。
  - 認証済みでアクセスした場合はホーム等の認証済みトップへリダイレクトする。
  - ログイン・サインアップ画面へ遷移する導線を必ず設置する。
- **設定画面 (`/settings`)** *(実装予定)*
...
- **認証ガードと遷移ポリシー**
  - 未認証: 保護ルートへアクセスされた場合は LP を表示し、ログイン画面は LP から導く。
  - 認証済み: `/login` や `/signup` へアクセスした際はホームへリダイレクトする。
  - ログアウト: 成功時はログイン画面へ遷移する。
