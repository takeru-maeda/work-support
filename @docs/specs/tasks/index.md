...
### 3.10. ユーザー削除機能
- [x] Supabase Auth のユーザー削除 API を呼び出す `/api/users/me` (DELETE) を実装する
- [x] `DELETE /api/users/me` のハンドラーで認証済みユーザー ID を取得し、Service Role キーで Supabase 管理 API を呼び出す
- [x] 削除成功時はステータス `204` を返し、フロントエンドがサインアウトできるようにする
...
- [x] ローカル保存を実装する
- [x] 実績工数 (`hours`) の必須入力バリデーションを実装する
...
- [x] ソート・フィルタ操作を実装する
- [x] `GET /api/reports/work-records` を呼び出し、取得データを一覧表示する
- [x] 絞り込み・並び替え条件を `localStorage` に保存し、復元できるようにする
...
### 4.10. プロフィール画面 (`/profile`) - ユーザー削除
- [x] プロフィール画面に「アカウント削除」ボタンと確認ダイアログを追加する
- [x] ダイアログでメールアドレス入力と一致チェックを行い、一致したときのみ削除ボタンを有効化する
- [x] ダイアログの削除アクションで `DELETE /api/users/me` を呼び出し、成功後にサインアウトして `/login` へ遷移する

### 4.11. 未認証時の LP 表示と導線
- [x] 未認証時にログイン画面ではなく LP を表示する
- [x] LP にログイン/新規登録への導線を設置する
- [x] ログアウト後はログイン画面へ遷移させる
